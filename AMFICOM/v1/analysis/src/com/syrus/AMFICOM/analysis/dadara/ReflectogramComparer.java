...
//////////////////
	...
	{
		this.reData = reData;
		this.data = ReflectogramMath.getReflectogrammFromEvents(reData, etalon[etalon.length-1].end);
		this.etalon = etalon;
		this.performShifting = performShifting;
	...
	{
		double []etalonArray = ReflectogramMath.getReflectogrammFromEvents(etalon, etalon[etalon.length-1].end);

		for(int i=0; i<hardAlarms.length; i++) // iniciating of the hard alarms properties;
		{
			ReflectogramAlarm ra = hardAlarms[i];
			ra.setEventType(ReflectogramMath.getEventType(ra.alarmPointCoord, etalon));
...
//      {
				...
				int number = ReflectogramMath.getEventNumber(ra.alarmPointCoord, etalon);
				for(int j=number+1; j<etalon.length; j++)
				{
					if(etalon[j].getType() == ReflectogramEvent.CONNECTOR)
					{
...
//
	...
	private void iniciateAlarms()
	{
		...
		for(int i=0; i<length; i++) // iniciating of the hard alarms;
		{
			...
			if(end-begin>minimalEventSize)
			{
				...
				if (ev != null &&
						(ev.getType() == ReflectogramEvent.CONNECTOR ||
						ev.getType() == ReflectogramEvent.WELD))
					begin = ev.begin;
	...
	{
		if(etalon[0].getType() == ReflectogramEvent.CONNECTOR)
		{
...
//-----------------------------------------------------------------------------
	 public static double getLoss(ReflectogramEvent []etalon, ReflectogramEvent []data, int nEvent)
	 {
		 ...
		 int coord = ReflectogramMath.getEventCenter(d);
...
//-----------------------------------------------------------------------------
	 ...
	 {
		 int coord = ReflectogramMath.getEventCenter(data[nEvent]);
		 ReflectogramEvent e = ReflectogramMath.getEvent(coord, etalon);
		 if(e == null)
			 return 0.;

		 int coord2 = ReflectogramMath.getEventCenter(e);
