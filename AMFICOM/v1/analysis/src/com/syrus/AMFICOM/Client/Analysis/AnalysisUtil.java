...
public class AnalysisUtil {
	...
	/**
	 * <p>
	 * ьФПФ НЕФПД ЙУРПМШЪХЕФ
	 * {@link ActionParameter#valueOf(Identifier, byte[], ActionParameterTypeBinding)},
	 * ДМС ФПЗП ЮФПВЩ РПРЩФБФШУС ОБКФЙ УХЭЕУФЧХАЭЙК РБТБНЕФТ, Х ЛПФПТПЗП
	 * ЧЕМЙЮЙОБ ТБЧОБ ЪБДБООПК. еУМЙ РПЙУЛ ОЕ ХЧЕОЮБМУС ХУРЕИПН, ФП ВХДЕФ УПЪДБО
	 * ОПЧЩК РБТБНЕФТ У ЪБДБООПК ЧЕМЙЮЙОПК. уМЕДПЧБФЕМШОП, Ч ЬФПН НЕФПДЕ
	 * РТПЙУИПДЙФ РПВБКФОПЕ УТБЧОЕОЙЕ НБУУЙЧПЧ. чПЪНПЦОП, ДМС ВПМШЫЙИ НБУУЙЧПЧ,
	 * ФБЛПЕ РПЧЕДЕОЙЕ ПЛБЦЕФУС ОЕЦЕМБФЕМШОЩН, Й ФПЗДБ РТЙДЈФУС ЪБНЕОЙФШ
	 * {@link ActionParameter#valueOf(Identifier, byte[], ActionParameterTypeBinding)}
	 * ОБ {@link ActionParameter#createInstance(Identifier, byte[], Identifier)}.
	 * 
	 * @param userId
	 * @param portType
	 * @param analysisParams
	 * @return рБТБНЕФТ БОБМЙЪБ
	 * @throws ApplicationException
	 */
	 ...
	 * @param portType
	 * @param bellcore
	 * @return рБТБНЕФТ ДМС ЬФБМПООПК ТЕЖМЕЛФПЗТБННЩ
	 * @throws ApplicationException
	 */
	 ...
	 * @param userId
	 * @param portType
	 *        тип порта, на котором будет проводиться этот анализ (в модуле
	 *        анализа он берется из типа порта, соответствующего шаблону, по
	 *        которому проведено измерение)
	 * @param meIds
	 *        множество MonitoredElements (вызывающая сторона сама должна
	 *        следить за согласованностью этого параметра с типом порта)
	 * @param withEtalon
	 *        true, чтобы в создаваемый шаблон включить эталон
