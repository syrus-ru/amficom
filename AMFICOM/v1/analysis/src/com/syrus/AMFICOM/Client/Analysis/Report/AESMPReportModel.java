...
public abstract class AESMPReportModel extends ReportModel
{
	...
	public RenderingComponent createReport(
		...
		if (data instanceof AbstractTableModel) {
			...
			try {
				MeasurementResultParameter measurementResult = null;
				StorableObjectCondition condition2 = new LinkedIdsCondition(
						measurementId, ObjectEntities.MEASUREMENTRESULTPARAMETER_CODE);
				Set<MeasurementResultParameter> resultSet = StorableObjectPool.getStorableObjectsByCondition(condition2, true);
				Iterator<MeasurementResultParameter> resultsIterator = resultSet.iterator();
				...
				BellcoreStructure bStructure =
					AnalysisUtil.getBellcoreStructureFromMeasurementResult(
							measurementResult);
