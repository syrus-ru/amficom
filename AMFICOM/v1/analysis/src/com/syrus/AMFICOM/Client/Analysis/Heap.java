...
/**
 * ¬ременна€ замена клиентскому пулу com.syrus.AMFICOM.Client.Resource.Pool
 * пока что API этой замены избыточна - например, getAllBSMap() делает необ€зательным
 * использование остальных методов работы с BS
 * 
 * ќбозначение: {} = хеш.
 * 
 * ќбладает свойствами, по которым не обеспечивает уведомлений:
 ...
 * newMSName
 * 
 * —войства, по которым уведомлени€ предусмотрены, но не систематизированы
 * (и не гарантированы):
 * bsHash{}
 * 
 * —войства с полным отслеживанием и уведомлени€ми:
 * refAnalysisPrimary;
 * etalonMTM;
 * currentTrace;
 * currentEvent, currentEtalonEvent;
 *
 *  роме того, есть свойство primaryMTAE, которое измен€етс€ вместе и только
 * вместе с refAnalysisPrimary; по его изменению тоже рассылаютс€ уведомлени€.
 * –ассылка происходит одновременно с рассылкой дл€ refAnalysisPrimary,
 * т.е. достаточно подписатьс€ только на одно из них. –екомендуетс€
 * подписыватьс€ на primaryMTAE в тех случа€х, когда refAnalysisPrimary не
 * нужен, а на refAnalysisPrimary - в случа€х, когда refAnalysisPrimary нужен.
 * ‘актически, primaryMTAE - это часть refAnalysisPrimary.
 * 
 * @author $Author: saa $
