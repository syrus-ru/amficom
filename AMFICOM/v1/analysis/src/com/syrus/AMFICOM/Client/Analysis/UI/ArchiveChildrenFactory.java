/*-
 * $Id: ArchiveChildrenFactory.java,v 1.27.2.1 2006/04/06 13:01:43 saa Exp $
 *
...
/**
 * Класс неизвестного назначения. // saa, 20060406
 * 
 * @version $Revision: 1.27.2.1 $, $Date: 2006/04/06 13:01:43 $
 * @author $Author: saa $
		...
		if (nodeObject instanceof String) {
			...
			try {
				...
				if (!measurementIds.isEmpty()) {
					condition.setEntityCode(ObjectEntities.MEASUREMENTRESULTPARAMETER_CODE);
					condition.setLinkedIdentifiables(measurementIds);
					Set<MeasurementResultParameter> resultSet =
						StorableObjectPool.getStorableObjectsByCondition(condition, true);
					for (MeasurementResultParameter result: resultSet) {
						//
