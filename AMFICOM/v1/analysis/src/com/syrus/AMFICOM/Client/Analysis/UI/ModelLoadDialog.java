/*-
 * $Id: ModelLoadDialog.java,v 1.1 2006/03/22 14:07:51 stas Exp $
 *
...
public class ModelLoadDialog {
	...
	private static Set<Identifier> selectedModelIds;
	...
	public static int showDialog(JFrame frame) {
		if (treeFilterUI == null) {
			...
			selectedModelIds = new HashSet<Identifier>();
		...
		if (result == JOptionPane.OK_OPTION) {
			...
			if (paths != null) {
				for (int i = 0; i < paths.length; i++) {
					for (int j = 0; j < paths[i].getPathCount(); j++) {
						...
						} else if (item.getObject() instanceof Modeling) {
			...
			try {
				...
				if (!selectedModelIds.isEmpty()) {
					StorableObjectCondition condition2 = new LinkedIdsCondition(selectedModelIds, ObjectEntities.RESULT_CODE);
					Set<StorableObject> resultSet = StorableObjectPool.getStorableObjectsByCondition(condition2, true);
					for (Iterator<StorableObject> iter = resultSet.iterator(); iter.hasNext();) {
						Result res = (Result) iter.next();
						if (res.getSort().value() == ResultSort.RESULT_SORT_MODELING.value()) {
							selectedResults.add(res);	
						}
					}
				}
