/*-
 * $Id: SchemeOptimizeInfoRtuFactory.java,v 1.3 2005/06/01 18:49:46 bass Exp $
 *
 ...
 * @module scheme_v1
 */
final class SchemeOptimizeInfoRtuFactory extends StorableObjectFactory {
	/**
	 * @param transferable
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new SchemeOptimizeInfoRtu((SchemeOptimizeInfoRtu_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((SchemeOptimizeInfoRtu_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new SchemeOptimizeInfoRtu_Transferable[length];
	}
}
