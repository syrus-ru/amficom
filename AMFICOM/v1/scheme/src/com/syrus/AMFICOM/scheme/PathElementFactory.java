/*-
 * $Id: PathElementFactory.java,v 1.4 2005/06/24 14:13:38 bass Exp $
 *
...
final class PathElementFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new PathElement((IdlPathElement) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((IdlPathElement) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new IdlPathElement[length];
	}
}
