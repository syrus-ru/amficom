/*-
 * $Id: SchemeProtoGroupFactory.java,v 1.4 2005/06/24 14:13:38 bass Exp $
 *
...
final class SchemeProtoGroupFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new SchemeProtoGroup((IdlSchemeProtoGroup) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((IdlSchemeProtoGroup) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new IdlSchemeProtoGroup[length];
	}
}
