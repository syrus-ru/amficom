/*-
 * $Id: SchemeOptimizeInfoSwitchFactory.java,v 1.3 2005/06/01 18:49:46 bass Exp $
 *
...
final class SchemeOptimizeInfoSwitchFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new SchemeOptimizeInfoSwitch((SchemeOptimizeInfoSwitch_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((SchemeOptimizeInfoSwitch_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new SchemeOptimizeInfoSwitch_Transferable[length];
	}
}
