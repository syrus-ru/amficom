/*-
 * $Id: SchemeOptimizeInfoFactory.java,v 1.4 2005/06/24 14:13:38 bass Exp $
 *
...
final class SchemeOptimizeInfoFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new SchemeOptimizeInfo((IdlSchemeOptimizeInfo) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((IdlSchemeOptimizeInfo) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new IdlSchemeOptimizeInfo[length];
	}
}
