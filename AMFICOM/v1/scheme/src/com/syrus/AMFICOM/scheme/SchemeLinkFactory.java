/*-
 * $Id: SchemeLinkFactory.java,v 1.4 2005/06/24 14:13:38 bass Exp $
 *
...
final class SchemeLinkFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new SchemeLink((IdlSchemeLink) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((IdlSchemeLink) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new IdlSchemeLink[length];
	}
}
