/*-
 * $Id: SchemePathFactory.java,v 1.4 2005/06/24 14:13:38 bass Exp $
 *
 ...
 * @module scheme_v1
 */
final class SchemePathFactory extends StorableObjectFactory {
	/**
	 * @param transferable
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new SchemePath((IdlSchemePath) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((IdlSchemePath) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new IdlSchemePath[length];
	}
}
