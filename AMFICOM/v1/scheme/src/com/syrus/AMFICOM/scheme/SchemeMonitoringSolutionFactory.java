/*-
 * $Id: SchemeMonitoringSolutionFactory.java,v 1.3 2005/06/01 18:49:47 bass Exp $
 *
 ...
 * @author $Author: bass $
 * @version $Revision: 1.3 $, $Date: 2005/06/01 18:49:47 $
 * @module scheme_v1
 */
final class SchemeMonitoringSolutionFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new SchemeMonitoringSolution((SchemeMonitoringSolution_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((SchemeMonitoringSolution_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new SchemeMonitoringSolution_Transferable[length];
	}
}
