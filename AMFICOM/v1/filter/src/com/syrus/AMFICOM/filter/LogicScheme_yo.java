...
//    this.restoreSchemeByText();
	...
	public ProSchemeElement_yo identifyObject (int x, int y)
	{
		...
		{
			LogicSchemeElement_yo se = (LogicSchemeElement_yo )schemeElements.get(i);
			if ((se.x < x) && (x < se.x + se.width) &&
				(se.y < y) && (y < se.y + se.height))
				return se;
		}
	...
	{
		if (/*(this.getRestrictionsNumber() == 0) ||*/
			(getFilterExpressions().size() < getRestrictionsNumber()))
		return false;
...
//			if (lse.filterExpression.equals(fe_old))
	...
	{
		this.schemeElements = new Vector();  
		this.finishedLinks = new Vector();
		this.activeZones = new Vector();
	}
	...
	public void restoreSchemeByLogic (String expression, Vector filterExpressions)
	{
		finishedLinks.removeAllElements();
		activeZones.removeAllElements();
...
//		setSchemeCoords();
	...
	*/
	...
	private FilterExpressionInterface getFEforListID(Vector filterExpressions, int listID)
	{
		for (int i = 0; i < filterExpressions.size(); i++)
		{
			FilterExpressionInterface fe = (FilterExpressionInterface )filterExpressions.get(i);
		...
		throws IOException
	{
		...
		for (int i = 0; i < schemeElements.size(); i++)
		{
			LogicSchemeElement_yo se = (LogicSchemeElement_yo )this.schemeElements.get(i);
			if (se.type.equals(LogicSchemeElement_yo.t_condition))
...
//		out.writeObject(filterExpressions); // Сохранили expressionы из этой схемы
		...
		String general_expression = this.getLogicString();
			...
			throws IOException, ClassNotFoundException
	{
	    ...
	    this.restoreSchemeByLogic(generalExpression, filterExpressions);
	}
}