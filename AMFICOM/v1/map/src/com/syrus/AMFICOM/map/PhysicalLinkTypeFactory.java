/*-
 * $Id: PhysicalLinkTypeFactory.java,v 1.3 2005/06/01 18:49:46 bass Exp $
 *
...
final class PhysicalLinkTypeFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new PhysicalLinkType((PhysicalLinkType_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((PhysicalLinkType_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new PhysicalLinkType_Transferable[length];
	}
}
