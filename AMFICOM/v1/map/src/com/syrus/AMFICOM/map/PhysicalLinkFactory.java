/*-
 * $Id: PhysicalLinkFactory.java,v 1.4 2005/06/24 10:41:46 bass Exp $
 *
 ...
 * @module map_v1
 */
final class PhysicalLinkFactory extends StorableObjectFactory {
	/**
	 * @param transferable
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new PhysicalLink((IdlPhysicalLink) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((IdlPhysicalLink) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new IdlPhysicalLink[length];
	}
}
