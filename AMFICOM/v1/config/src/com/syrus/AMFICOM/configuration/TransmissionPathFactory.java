/*-
 * $Id: TransmissionPathFactory.java,v 1.3 2005/06/22 10:05:17 bass Exp $
 *
...
final class TransmissionPathFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new TransmissionPath((IdlTransmissionPath) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((IdlTransmissionPath) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new IdlTransmissionPath[length];
	}
}
