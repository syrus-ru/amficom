/*-
 * $Id: MeasurementPortTypeFactory.java,v 1.2 2005/06/01 18:49:44 bass Exp $
 *
 ...
 * @module config_v1
 */
final class MeasurementPortTypeFactory extends StorableObjectFactory {
	/**
	 * @param transferable
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new MeasurementPortType((MeasurementPortType_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((MeasurementPortType_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new MeasurementPortType_Transferable[length];
	}
}
