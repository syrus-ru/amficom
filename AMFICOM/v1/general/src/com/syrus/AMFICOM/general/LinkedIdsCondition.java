/*
 * $Id: LinkedIdsCondition.java,v 1.1 2004/12/23 12:25:27 bob Exp $
 *
...
/**
 * If one needs to write a <code>LinkedIdsCondition</code> for a certain
 ...
 * <code>LinkedIdsCondition</code> and residing in the appropriate package
 ...
 * {@link LinkedIdsCondition com.syrus.AMFICOM.general.LinkedIdsCondition})</li>
 * <li>It must declare <em>the only</em> <code>private</code> constructor with
 * the following code:<pre>
 * private LinkedIdsCondition(final Identifier identifier, final Short entityCode) {
 ...
 * 
 * </pre></li>
 * <li>It must override {@link #isConditionTrue(Object)}, {@link #isNeedMore(List)} and {@link #setEntityCode(Short)}.</li></ul> 
 *
 * @author $Author: bob $
 * @version $Revision: 1.1 $, $Date: 2004/12/23 12:25:27 $
 * @module general_v1
 */
public class LinkedIdsCondition implements StorableObjectCondition {
	...
	public LinkedIdsCondition(final Identifier identifier, final Short entityCode) {
		...
		try {
			...
			this.delegate = (LinkedIdsCondition) ctor.newInstance(
				new Object[]{identifier, entityCode});
			...
			Log.debugMessage(LINKED_IDS_CONDITION_INIT
				...
				+ LinkedIdsCondition.class.getName()
				+ CREATING_A_DUMMY_CONDITION,
				Log.WARNING);
			...
			if (this.delegate == null) {
				this.delegate = new LinkedIdsCondition() {
					public boolean isConditionTrue(final Object object) throws ApplicationException {
						Log.debugMessage(
							LINKED_IDS_CONDITION_INNER_ONE_IS_CONDITION_TRUE
	...
	public LinkedIdsCondition(final List linkedIds, final Short entityCode) {
		...
		try {
			...
			ctor.setAccessible(true);
			this.delegate = (LinkedIdsCondition) ctor.newInstance(
				new Object[]{linkedIds, entityCode});
		} catch (ClassNotFoundException cnfe) {
			Log.debugMessage(LINKED_IDS_CONDITION_INIT
				+ "Class " + className //$NON-NLS-1$
				+ " not found on the classpath" //$NON-NLS-1$
	...
	public LinkedIdsCondition(final LinkedIdsCondition_Transferable transferable) {		
		...
		if (transferable.linked_ids.length == 1) {
			this.delegate.identifier = new Identifier(transferable.linked_ids[0]);
		} else {
			this.delegate.linkedIds = new ArrayList(transferable.linked_ids.length);
			for (int i = 0; i < transferable.linked_ids.length; i++) {
				this.delegate.linkedIds.add(new Identifier(transferable.linked_ids[i]));
			}
		}
	...
	public final Object getTransferable() {
		...
		transferable.domain_id = (Identifier_Transferable) this.delegate.domainId.getTransferable();
	...
	public final void setDomainId(final Identifier domainId) {
		this.delegate.domainId = domainId;
	}
	
	public final Identifier getDomainId() {
		return this.delegate.domainId;
	}
