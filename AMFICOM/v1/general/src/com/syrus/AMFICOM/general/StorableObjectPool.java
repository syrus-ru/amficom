...
//				Log.debugMessage("StorableObjectPool.addObjectPool | pool for " + ObjectEntities.codeToString(short1.shortValue()) + "/" + short1 + "(" + objectEntityCode +") size  " + poolSize + " added", Log.DEBUGLEVEL07);
				...
				throw new UnsupportedOperationException(this.selfGroupName + "StorableObjectPool.addObjectPool | CacheMapClass " + this.cacheMapClass.getName() + " must extend LRUMap");
		}
		catch (SecurityException e) {
			throw new UnsupportedOperationException(this.selfGroupName + "StorableObjectPool.addObjectPool | CacheMapClass " + this.cacheMapClass.getName() + " SecurityException " + e.getMessage());
		}
		catch (IllegalArgumentException e) {
			throw new UnsupportedOperationException(this.selfGroupName + "StorableObjectPool.addObjectPool | CacheMapClass " + this.cacheMapClass.getName() + " IllegalArgumentException " + e.getMessage());
		}
		catch (NoSuchMethodException e) {
			throw new UnsupportedOperationException(this.selfGroupName + "StorableObjectPool.addObjectPool | CacheMapClass " + this.cacheMapClass.getName() + " NoSuchMethodException " + e.getMessage());
		}
		catch (InstantiationException e) {
			throw new UnsupportedOperationException(this.selfGroupName + "StorableObjectPool.addObjectPool | CacheMapClass " + this.cacheMapClass.getName() + " InstantiationException " + e.getMessage());
		}
		catch (IllegalAccessException e) {
			throw new UnsupportedOperationException(this.selfGroupName + "StorableObjectPool.addObjectPool | CacheMapClass " + this.cacheMapClass.getName() + " IllegalAccessException " + e.getMessage());
		}
		catch (InvocationTargetException e) {
			throw new UnsupportedOperationException(this.selfGroupName + "StorableObjectPool.addObjectPool | CacheMapClass " + this.cacheMapClass.getName() + " InvocationTargetException " + e.getMessage());
		}
	...
	protected synchronized void deleteImpl(final Identifier id) {
		...
		if (lruMap != null)
			...
			Log.errorMessage(this.selfGroupName + "StorableObjectPool.deleteImpl | Cannot find object pool for entity '" + ObjectEntities.codeToString(entityCode.shortValue()) + "' entity code: " + entityCode);
	...
	private void saveWithDependencies(StorableObject storableObject, int dependencyLevel) throws ApplicationException {
	...
	protected static StorableObject getStorableObjectOfGroup(Identifier id, boolean useLoader, short groupCode) throws ApplicationException {
		...
		try {
			...
			storableObject = (StorableObject) getStorableObjectMethod.invoke(null, new Object[] { id, useLoader ? Boolean.TRUE : Boolean.FALSE});
		}
	 ...
	 */
	protected Collection getStorableObjectsImpl(final Collection objectIds, final boolean useLoader) throws ApplicationException {
		List list = null;
	...
	protected abstract Collection loadStorableObjects(final Short entityCode, final Collection ids) throws ApplicationException;

	protected abstract Collection loadStorableObjectsButIds(final StorableObjectCondition condition, final Collection ids) throws ApplicationException;
...
//*/
		...
		throw new IllegalObjectEntityException(
			this.selfGroupName + "StorableObjectPool.putStorableObject | Illegal object entity: '"
				+ ObjectEntities.codeToString(entityCode) + "'",
	 ...
	 */
	protected abstract void saveStorableObjects(final short entityCode, final Collection storableObjects, final boolean force) throws ApplicationException;
