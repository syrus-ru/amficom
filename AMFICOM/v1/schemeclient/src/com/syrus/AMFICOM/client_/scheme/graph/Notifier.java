...
import com.syrus.AMFICOM.client_.scheme.ui.MultipleSelectionPropertiesManager;
...
//		TODO write visual managers
	...
	public static void notify(SchemeGraph graph, ApplicationContext aContext, Object[] cells) {
		try {
			...
			Set<Identifiable> selectedObjects = new HashSet<Identifiable>();
			for (Object cell : cells) {
				if (cell instanceof Identifiable) {
					selectedObjects.add((Identifiable)cell);
				}
			}
...
//			}
				...
				if (selectedType == 0 || selectedObject == null) {
					...
					if (object instanceof IdentifiableCell) {
						...
						dispatcher.firePropertyChange(new ObjectSelectedEvent(graph, (IdentifiableCell)object, null, OTHER_OBJECT));
						Log.debugMessage(message, Level.FINEST);
					} else {
						Log.debugMessage("Unsupported selection", Level.FINEST);
					}
				} else {
					if (SchemeGraph.getMode().equals(Constants.RACK_MODE)) {
