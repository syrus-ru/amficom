/*-
 * $Id: ElementsToolBar.java,v 1.1 2005/04/05 14:07:53 stas Exp $
 *
...
public class ElementsToolBar extends UgoToolBar {
	...
	protected UgoTabbedPane targetPane;
	...
	protected ElementsToolBar(UgoTabbedPane sourcePane, UgoTabbedPane targetPane,
	...
	protected Map createGraphButtons() {
		...
		if (pane.getGraph().getMarqueeHandler() instanceof ShemeMarqueeHandler) {
			ShemeMarqueeHandler mh = (ShemeMarqueeHandler) pane.getGraph()
					.getMarqueeHandler();

			bttns.put(Constants.marqueeTool,
					createToolButton(mh.s, btn_size, null, null, Constants.ICON_MARQUEE,
							new MarqeeAction(pane.getGraph()), true));
			...
			bttns.put(Constants.portOutKey,
					createToolButton(mh.p1, btn_size, null, Constants.TEXT_PORT,
							Constants.ICON_PORT, new PortToolAction(), false));
			...
			bttns.put(Constants.ungroupKey, createToolButton(mh.ugr, btn_size, null,
					Constants.TEXT_UNGROUP, Constants.ICON_UNGROUP, new UngroupAction(
							pane), false));
			bttns.put(Constants.undoKey,
					createToolButton(mh.undo, btn_size, null, Constants.TEXT_UNDO,
							Constants.ICON_UNDO, new UndoAction(pane), false));
			bttns.put(Constants.redoKey,
					createToolButton(mh.redo, btn_size, null, Constants.TEXT_REDO,
							Constants.ICON_REDO, new RedoAction(pane), false));
			bttns.put(Constants.zoomInKey, createToolButton(mh.zi, btn_size, null,
					Constants.TEXT_ZOOM_IN, Constants.ICON_ZOOM_IN,
					new ZoomInAction(pane), true));
			bttns.put(Constants.zoomOutKey, createToolButton(mh.zo, btn_size, null,
					Constants.TEXT_ZOOM_OUT, Constants.ICON_ZOOM_OUT, new ZoomOutAction(
							pane), true));
			bttns.put(Constants.zoomActualKey, createToolButton(mh.za, btn_size,
					null, Constants.TEXT_ZOOM_NORMAL, Constants.ICON_ZOOM_NORMAL,
