...
public class SchemeDevice extends StubResource implements Serializable
{
	...
	public boolean isCrossRouteValid()
	{
		...
		for (Iterator it = cableports.iterator(); it.hasNext(); )
		{
			...
			if (p.cableLinkId.length() != 0)
				cables.add(Pool.get(SchemeCableLink.typ, p.cableLinkId));
		}
		for (Iterator it = cables.iterator(); it.hasNext();)
		{
			SchemeCableLink cable = (SchemeCableLink)it.next();
			for (Iterator tit = cable.cableThreads.iterator(); tit.hasNext();)
	...
	public void createDefaultCrossRoute()
	{
		...
		for (Iterator it = cableports.iterator(); it.hasNext(); )
		{
			...
			if (p.cableLinkId.length() != 0)
			{
				if (p.directionType.equals("out"))
					outputCables.add(Pool.get(SchemeCableLink.typ, p.cableLinkId));
				else
					inputCables.add(Pool.get(SchemeCableLink.typ, p.cableLinkId));
			}
		}
	...
	private void findCrossRoute(Collection cables, String direction, PortThreadMap crossroute)
	{
		...
		for (Iterator it = ports.iterator(); it.hasNext(); )
		{
			...
			if (p.directionType.equals(direction))
				freePorts.add(p);
		}
	...
	public void updateLocalFromTransferable()
	{
		...
		{
			PortThreadMap_Transferable mapTr = (PortThreadMap_Transferable) transferable.portmap[i];
			SchemePort port = getPort(mapTr.portId);
			SchemeCableLink cable = (SchemeCableLink) Pool.get(SchemeCableLink.typ, mapTr.cableLinkId);
			crossroute.add(port, cable.getCableThread(mapTr.threadId));
		}
	}
	...
	public Object clone(DataSourceInterface dataSource)
	{
		...
		if (clonedId != null)
		{
			SchemeDevice cloned = (SchemeDevice)Pool.get(SchemeDevice.typ, clonedId);
			if (cloned == null)
