...
public class ComponentSaveCommand extends AbstractCommand {
	...
	public Object clone() {
		return new ComponentSaveCommand(aContext, cellPane);
	}
	...
	public void execute() {
		...
		if (cellPane.getCurrentPanel().getSchemeResource().getSchemeProtoElement() != null) {
			...
			if (groups.length > 1) {
				JOptionPane.showMessageDialog(Environment.getActiveWindow(), 
						"blah-blah-blah! Надо создать УГО или компонент должен быть простым!", Constants.ERROR, JOptionPane.OK_OPTION);
				return;
			}
			if ((status & SchemeActions.SCHEME_HAS_LINK) != 0) {
				JOptionPane.showMessageDialog(Environment.getActiveWindow(),
						"Простой компонент не может иметь линий связи", Constants.ERROR, JOptionPane.OK_OPTION);
				return;
			}
