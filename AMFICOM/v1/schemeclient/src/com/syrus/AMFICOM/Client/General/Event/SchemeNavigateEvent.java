...
public class SchemeNavigateEvent extends OperationEvent
{
	...
	public boolean SCHEME_DEVICE_SELECTED = false;
	...
	public static final long SCHEME_PROTO_ELEMENT_DESELECTED_EVENT = 0x00000400;
	...
	public void setTyp(long typ)
	{
		...
		else if((typ == SCHEME_ELEMENT_SELECTED_EVENT))
			SCHEME_ELEMENT_SELECTED = true;
		else if((typ == SCHEME_PROTO_ELEMENT_SELECTED_EVENT))
			SCHEME_PROTO_ELEMENT_SELECTED = true;
		else if((typ == SCHEME_PORT_SELECTED_EVENT))
			SCHEME_PORT_SELECTED = true;
		else if((typ == SCHEME_CABLE_PORT_SELECTED_EVENT))
			SCHEME_CABLE_PORT_SELECTED = true;
		else if((typ == SCHEME_LINK_SELECTED_EVENT))
			SCHEME_LINK_SELECTED = true;
		else if((typ == SCHEME_CABLE_LINK_SELECTED_EVENT))
			SCHEME_CABLE_LINK_SELECTED = true;
		else if((typ == SCHEME_DEVICE_SELECTED_EVENT))
			SCHEME_DEVICE_SELECTED = true;
		else if((typ == OTHER_OBJECT_SELECTED_EVENT))
			OTHER_OBJECT_SELECTED = true;

		else if((typ == SCHEME_PATH_DESELECTED_EVENT))
			SCHEME_PATH_DESELECTED = true;
		else if((typ == SCHEME_ELEMENT_DESELECTED_EVENT))
			SCHEME_ELEMENT_DESELECTED = true;
		else if((typ == SCHEME_PORT_DESELECTED_EVENT))
			SCHEME_PORT_DESELECTED = true;
		else if((typ == SCHEME_CABLE_PORT_DESELECTED_EVENT))
			SCHEME_CABLE_PORT_DESELECTED = true;
		else if((typ == SCHEME_LINK_DESELECTED_EVENT))
			SCHEME_LINK_DESELECTED = true;
		else if((typ == SCHEME_CABLE_LINK_DESELECTED_EVENT))
			SCHEME_CABLE_LINK_DESELECTED = true;
		else if((typ == SCHEME_ALL_DESELECTED_EVENT))
			SCHEME_ALL_DESELECTED = true;
		else if((typ == SCHEME_SELECTED_EVENT))
