...
public class Result extends ObjectResource implements Serializable
{
	...
	private transient static SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yy HH:mm:ss");
...
//
	...
	public void updateLocalFromTransferable()
	{
		...
		if (result_type.equals(Modeling.typ))
		{
			modeling_id = action_id;
		...
		if (result_type.equals(Evaluation.typ))
		{
		...
		if (result_type.equals(Test.typ))
		{
			test_id = action_id;
...
//				transferable.parameters[i] = (ClientParameter_Transferable)((Parameter)parameters.get(i)).getTransferable();
			...
			Set keySet = map.keySet();
			transferable.parameters = new ClientParameter_Transferable[keySet.size()];
			int i=0;
			for (Iterator it = keySet.iterator(); it.hasNext();) {
...
//		return sdf.format(new Date(elementary_start_time));
	...
	/**
	 * @param modelingId The modelingId to set.
	 */
	...
	/**
	 * @param testRequestId The testRequestId to set.
	 */
