...
import com.syrus.AMFICOM.Client.General.Scheme.AlarmedPathAnimator;
...
//		setSize(new Dimension(960, 650));
		this.setTitle(LangModelSurvey.String("AppTitle"));
		this.addComponentListener(new SurveyMDIMain_this_componentAdapter(this));
...
*/
		...
		aModel.setCommand("menuEvaluateArchive", new ViewSurveyNavigatorCommand(
				internal_dispatcher,
				desktopPane,
				"Навигатор объектов",
				aContext));

		aModel.setCommand("menuEvaluateScheduler", new OpenSchedulerCommand(Environment.the_dispatcher, aContext, new DefaultScheduleApplicationModelFactory()));
//		aModel.setCommand("menuEvaluateRequest", new TestRequestCommand(aContext));

		aModel.setCommand("menuEvaluateTrackRequest", new OpenTestsCommand(internal_dispatcher, desktopPane, aContext, "testrequest"));
		aModel.setCommand("menuEvaluateTrackTask", new OpenTestsCommand(internal_dispatcher, desktopPane, aContext, "test"));

		aModel.setCommand("menuEvaluateResult", new OpenResultsCommand(internal_dispatcher, desktopPane, aContext, new DefaultResultApplicationModelFactory()));
		aModel.setCommand("menuEvaluateAnalize", new OpenAnalysisCommand(Environment.the_dispatcher, aContext, new ReflectometryAnalyseApplicationModelFactory()));
		aModel.setCommand("menuEvaluateAnalizeExt", new OpenExtendedAnalysisCommand(Environment.the_dispatcher, aContext, new ReflectometryAnalyseApplicationModelFactory()));
		aModel.setCommand("menuEvaluateNorms", new OpenNormsCommand(Environment.the_dispatcher, aContext, new ReflectometryAnalyseApplicationModelFactory()));
		aModel.setCommand("menuEvaluateModeling", new OpenModelingCommand(Environment.the_dispatcher, aContext, new DefaultModelApplicationModelFactory()));
		aModel.setCommand("menuEvaluatePrognosis", new OpenPrognosisCommand(Environment.the_dispatcher, aContext, new DefaultPredictionApplicationModelFactory()));
		aModel.setCommand("menuEvaluateViewAll", new SurveyViewAllCommand(internal_dispatcher, desktopPane, aContext));

		aModel.setCommand("menuVisualizeNet", new SurveyViewAllSchemeCommand(internal_dispatcher, desktopPane, aContext));
		aModel.setCommand("menuVisualizeNetGIS", new SurveyViewAllCommand(internal_dispatcher, desktopPane, aContext));
		aModel.setCommand("menuVisualizeMapEdit", new OpenMapEditorCommand(Environment.the_dispatcher, aContext, null));
		aModel.setCommand("menuVisualizeMapClose", new MapCloseCommand(internal_dispatcher, aContext));
											//new SurveyNewMapViewCommand(internal_dispatcher, desktopPane, aContext, new MapSurveyNetApplicationModelFactory()));
		aModel.setCommand("menuVisualizeISMGIS", new MapJViewOpenCommand(internal_dispatcher, desktopPane, aContext, new MapSurveyISMApplicationModelFactory()));
		aModel.setCommand("menuVisualizeSchemeEdit", new OpenSchemeEditorCommand(internal_dispatcher, aContext, new SchematicsApplicationModelFactory()));
		aModel.setCommand("menuVisualizeSchemeClose", new SchemeCloseCommand(aContext, null));
		aModel.setCommand("menuViewAll", new SurveyViewAllCommand(internal_dispatcher, desktopPane, aContext));
		aModel.setCommand("menuViewMapSetup", new SurveyViewMapSetupCommand(desktopPane, aContext));

		aModel.setCommand("menuMaintainAlarm", new OpenAlarmsCommand(internal_dispatcher, desktopPane, aContext, new DefaultAlarmApplicationModelFactory()));
	...
	public void setContext(ApplicationContext aContext)
	{
		if(this.aContext!= null)
			if(this.aContext.getDispatcher() != null)
			{
				...
				internal_dispatcher.unregister(this, SchemeElementsEvent.type);
				internal_dispatcher.unregister(this, "addschemeelementevent");
				internal_dispatcher.unregister(this, "addschemeevent");
				Environment.the_dispatcher.unregister(this, "mapaddschemeevent");
				Environment.the_dispatcher.unregister(this, "mapaddschemeelementevent");
			}
...
//			if (aModel.isEnabled("mapActionReload"))
			{
				...
				myalarmTread = new ReloadAttributes((MapMainFrame )frame);//Запускаем thread
				myalarmTread.start();
			}

		}
		else
		if(ae.getActionCommand().equals("mapjframeshownevent"))
		{
			JInternalFrame frame = (JInternalFrame )ae.getSource();
...
/*			
				if (schemeAlarmUpdater == null)
				{
					for(int i = 0; i < desktopPane.getComponents().length; i++)
					{
						...
						{
							SchemeViewerFrame frame = (SchemeViewerFrame)comp;
							if (frame.panel instanceof SchemePanel)
							{
								schemeAlarmUpdater = new SchemeAlarmUpdater(aContext, (SchemePanel)frame.panel);
								schemeAlarmUpdater.start();
//								frame.startPathAnimator();
				...
				else if (!schemeAlarmUpdater.flag)
					schemeAlarmUpdater.start();
*/
				ApplicationModel aModel = aContext.getApplicationModel();
...
//		alarmChecker = new AlarmChecker(internal_dispatcher, aContext, this, desktopPane);
//		alarmChecker.start();
	}
...
class SurveyMDIMain_this_componentAdapter extends java.awt.event.ComponentAdapter
{
