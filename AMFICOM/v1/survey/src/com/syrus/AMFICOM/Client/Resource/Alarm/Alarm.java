...
public class Alarm extends ObjectResource implements Serializable {
	...
	public static Alarm getAlarmByTestResult(String result_id) {
		...
		for (res_index = 0; res_index < test.result_ids.length; res_index++)
			if (test.result_ids[res_index].equals(result_id)) break;
		if (res_index == test.result_ids.length) return null;
		...
		for (Enumeration e = ht.elements(); e.hasMoreElements();) {
			...
			Evaluation eval = (Evaluation) Pool.get("evaluation", rrr
					.getActionId());
			if (eval == null) continue;
			if (test.evaluation_id.equals(rrr.getActionId())) {
				if (eval.getResultIds().length > res_index)
						if (eval.getResultIds()[res_index]
								.equals(event.descriptor)) return a;
			}
		}
		return null;
	}
		...
		try {
			...
			if (event.type_id.equals("evaluationalarmevent")) {
				Evaluation ev = (Evaluation) Pool.get("evaluation",
						event.descriptor);
				return ev;
			} else if (event.type_id.equals("testalarmevent")
	...
	public ObjectResource getElementaryTest() {
		try {
			...
			if (event.type_id.equals("evaluationalarmevent")) {
				/*
				 * Evaluation ev = (Evaluation )Pool.get("evaluation",
				 * event.descriptor); Result result = (Result )Pool.get("result",
				 * ev.result_id); if(!result.result_type.equals("test")) return null;
				 * Test t = (Test )Pool.get("test", result.test_id); return t;
				 */
				return null;
			} else if (event.type_id.equals("testalarmevent")
					|| event.type_id.equals("testwarningevent")) {
	...
	public Evaluation getEvaluation() {
		...
		Evaluation eval = (Evaluation) Pool
				.get("evaluation", rrr.getActionId());
		return eval;
	}
	...
	public String getMonitoredElementId() {
		try {
			...
			if (event.type_id.equals("evaluationalarmevent")) {
				...
				if (res == null) return "";
				if (res.getResultType().equals("test")) {
					Test test = (Test) Pool.get(Test.typ, res.getActionId());
					if (test == null) return "";
					return test.monitored_element_id;
				}
				if (res.getResultType().equals("evaluation")) {
			...
			if (event.type_id.equals("evaluationalarmevent")) {
				for (Enumeration enum = Pool.getHash("result").elements(); enum
					...
					if (r.getResultType().equals("evaluation")
							&& r.getId().equals(event.descriptor)) return r;
				}
				return null;
			} else if (event.type_id.equals("testalarmevent")
					|| event.type_id.equals("testwarningevent")) {
	...
	public String getSourceId() {
		try {
			SystemEvent event = (SystemEvent) Pool.get("event", event_id);
			if (event != null) return event.source_id;
		} catch (Exception ex) {
			return null;
		}
		return null;
	...
	public ObjectResource getTest() {
		try {
			...
			if (event.type_id.equals("evaluationalarmevent")) {
				...
				//				  if(!result.result_type.equals("test")) return null; Test t = (Test
