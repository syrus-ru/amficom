...
// Delta_x
	...
	public ReflectogramEvent(byte[] b, boolean old) {
...
// Parameters for the Connectors
	...
	public static ReflectogramEvent[] fromByteArray(byte[] bar) {
		...
		DataInputStream dis = new DataInputStream(bais);
		byte[] buf = new byte[OLD_EVENT_SIZE];
		LinkedList ll = new LinkedList();
		try {
			while (dis.read(buf) == OLD_EVENT_SIZE)
				ll.add(new ReflectogramEvent(buf, true));
		}
		catch (IOException e) {
			e.printStackTrace();
		}
