...
//*-*          =======================================================
   ...
   fMaxpar      = mni;
   int mnihl  = mni*(mni+1)/2;
   int maxcpt = 101;
   int mne    = 2*mni;
   fCpnam  = new TString[mne];
   fU      = new double[mne];
   fAlim   = new double[mne];
   fBlim   = new double[mne];
   fErp    = new double[mni];
   fErn    = new double[mni];
   fWerr   = new double[mni];
   fGlobcc = new double[mni];
   fNvarl  = new int[mne];
   fNiofex = new int[mne];
   fNexofi = new int[mni];
   fX      = new double[mni];
   fXt     = new double[mni];
   fDirin  = new double[mni];
   fXs     = new double[mni];
   fXts    = new double[mni];
   fDirins = new double[mni];
   fGrd    = new double[mni];
   fG2     = new double[mni];
   fGstep  = new double[mni];
   fGin    = new double[mne];
   fDgrd   = new double[mni];
   fGrds   = new double[mni];
   fG2s    = new double[mni];
   fGsteps = new double[mni];
   fIpfix  = new int[mni];
   fVhmat  = new double[mnihl];
   fVthmat = new double[mnihl];
   fP      = new double[mni*(mni+1)];
   fPstar  = new double[2*mni];
   fPstst  = new double[mni];
   fPbar   = new double[mni];
   fPrho   = new double[mni];
   fWord7  = new double[mni];
   fXpt    = new double[maxcpt];
   fYpt    = new double[maxcpt];
   ...
   fCONTgcc   = new double[mni];
   fCONTw     = new double[mni];
   fFIXPyy    = new double[mni];
   fGRADgf    = new double[mni];
   fHESSyy    = new double[mni];
   fIMPRdsav  = new double[mni];
   fIMPRy     = new double[mni];
   fMATUvline = new double[mni];
   fMIGRflnu  = new double[mni];
   fMIGRstep  = new double[mni];
   fMIGRgs    = new double[mni];
   fMIGRvg    = new double[mni];
   fMIGRxxs   = new double[mni];
   fMNOTxdev  = new double[mni];
   fMNOTw     = new double[mni];
   fMNOTgcc   = new double[mni];
   fPSDFs     = new double[mni];
   fSEEKxmid  = new double[mni];
   fSEEKxbest = new double[mni];
   fSIMPy     = new double[mni];
   fVERTq     = new double[mni];
   fVERTs     = new double[mni];
   fVERTpp    = new double[mni];
   fCOMDplist = new double[mni];
   fPARSplist = new double[mni];
...
//   printf("Deleting of the objects in progress...");
//   if(true) return;
   if (fEmpty) return;

//   printf("Deleting of the objects in progress...");
//   delete [] fCpnam; // = new TString[mni];
   delete [] fU;
   delete [] fAlim;
   delete [] fBlim;
   delete [] fErp;
...
//   delete [] fOrigin;// = new TString[100];
//   delete [] fWarmes;// = new TString[100];
   // initialisation of dynamic arrays used internally in some functions
   // these arrays had a fix dimension in Minuit
   delete [] fCONTgcc;
   delete [] fCONTw;
...
// fix a parameter
     ...
     int err;
     double tmp = parNo+1; //set internal Minuit nuumbering
...
//______________________________________________________________________________
int TMinuit::GetParameter( int parNo, double &currentValue, double &currentError )
{
...
// release a parameter
     ...
     int err;
     double tmp = parNo+1; //set internal Minuit numbering
...
L20:
    ...
    log_ = int(log10(awid));
    if (awid <= 1) --log_;
    sigfig = awid*pow(10., (double)(-log_));
...
L60:
    ...
    ++log_;
L100:
    bwid = sigrnd*pow(10., (double)(log_));
    goto L200;
...
//______________________________________________________________________________
void TMinuit::mncrck(TString cardbuf, int maxcwd, TString &comand, int &lnc,
...
//*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    ...
    int npfn, ndex, loop=0, i, j, ifail, iseed;
...
//*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
	...
	if (fNvarl[i-1] == 1) {
	    ...
	    fU[i-1] = fAlim[i-1] + (sin(pint[j-1]) + 1)*.5*(fBlim[i-1] - fAlim[i-1]);
	}
    }
} /* mninex_ */
...
//______________________________________________________________________________
void TMinuit::mnparm(int k1, TString cnamj, double uk, double wk, double a, double b, int &ierflg)
{
...
//______________________________________________________________________________
void TMinuit::mnpars(TString &crdbuf, int &icondn)
{
//*-*-*-*-*-*-*-*Implements one parameter definition*-*-*-*-*-*-*-*-*-*-*-*-*
//*-*            =========== =======================
...
//______________________________________________________________________________
void TMinuit::mnpout(int iuext1, TString &chnam, double &val, double &err, double &xlolim, double &xuplim, int &iuint)
{
...
//*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    ...
    double flast, bar;
    int ipar, iext, j, ifail, iseed, nparx, istep, ib, mxfail, mxstep;
...
//*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    ...
    int ikseed, idbopt, igrain, iswsav, isw2;
...
//______________________________________________________________________________
bool TMinuit::mnunpt(TString &cfname)
{
...
double TMinuit::Min(double a1, double a2)
 {
	 if(a1<a2)return a1;
	 return a2;
 }

int TMinuit::Min(int a1, int a2)
 {
	 if(a1<a2)return a1;
	 return a2;
 }

double TMinuit::Max(double a1, double a2)
 {
	 if(a1>a2)return a1;
	 return a2;
 }

int TMinuit::Max(int a1, int a2)
 {
	 if(a1>a2)return a1;
	 return a2;
 }
