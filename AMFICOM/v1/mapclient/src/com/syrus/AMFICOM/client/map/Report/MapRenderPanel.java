...
public class MapRenderPanel extends JPanel
{
	...
	{
		this.reportsRO = ro;
		String reserve_string = (String)reportsRO.getReportToRender().getReserve();
		...
		DataSourceInterface dataSource = aC.getApplicationModel().getDataSource(SessionInterface.getActiveSession());
		aC.setDataSourceInterface(dataSource);

		try
		{
			MapMainFrame mmf = new MapMainFrame(panelElements, aC);
			NetMapViewer nmv = mmf.myMapViewer;
