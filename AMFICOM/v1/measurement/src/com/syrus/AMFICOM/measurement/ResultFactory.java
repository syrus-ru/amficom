/*-
 * $Id: ResultFactory.java,v 1.2 2005/06/01 18:49:44 bass Exp $
 *
...
final class ResultFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new Result((Result_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((Result_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new Result_Transferable[length];
	}
}
