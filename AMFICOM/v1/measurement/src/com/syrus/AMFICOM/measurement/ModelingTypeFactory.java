/*-
 * $Id: ModelingTypeFactory.java,v 1.2 2005/06/01 18:49:44 bass Exp $
 *
...
final class ModelingTypeFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new ModelingType((ModelingType_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((ModelingType_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new ModelingType_Transferable[length];
	}
}
