/*-
 * $Id: MeasurementSetupFactory.java,v 1.2 2005/06/01 18:49:44 bass Exp $
 *
...
final class MeasurementSetupFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new MeasurementSetup((MeasurementSetup_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((MeasurementSetup_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new MeasurementSetup_Transferable[length];
	}
}
