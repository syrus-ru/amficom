/*-
 * $Id: CronTemporalPatternFactory.java,v 1.2 2005/06/01 18:49:44 bass Exp $
 *
 ...
 */
final class CronTemporalPatternFactory extends StorableObjectFactory {
	/**
	 * @param transferable
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new CronTemporalPattern((CronTemporalPattern_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((CronTemporalPattern_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new CronTemporalPattern_Transferable[length];
	}
}
