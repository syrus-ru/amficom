/*-
 * $Id: ModelChildrenFactory.java,v 1.1 2005/11/24 15:46:12 stas Exp $
 *
...
public class ModelChildrenFactory extends AbstractChildrenFactory {
	...
	private static final String	ROOT	= "tree.modeling.root";
	private static final String	MODELS	= "tree.modeling.models";
	private static final String	MONITOREDELEMENTS	= "tree.modeling.monitoredelements";
		...
		if (nodeObject instanceof String) {
			...
			if (s.equals(ROOT)) {
				if (item.getChildren().isEmpty()) { // add only if no children as they are constant
					item.addChild(new ResultChildrenFactory().getRoot());
					PopulatableIconedNode item2 = new PopulatableIconedNode();
					item2.setObject(MODELS);
					item2.setName(I18N.getString(MODELS));
				...
				if (item.getChildren().isEmpty()) { // add only if no children as they are constant
					...
					item2.setName(I18N.getString(MONITOREDELEMENTS));
					item2.setChildrenFactory(this);
