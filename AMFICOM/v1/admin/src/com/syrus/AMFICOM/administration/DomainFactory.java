/*-
 * $Id: DomainFactory.java,v 1.2 2005/06/01 18:49:45 bass Exp $
 *
...
final class DomainFactory extends StorableObjectFactory {
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new Domain((Domain_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((Domain_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new Domain_Transferable[length];
	}
}
