/*-
 * $Id: UserFactory.java,v 1.2 2005/06/01 18:49:45 bass Exp $
 *
 ...
 * @module admin_v1
 */
final class UserFactory extends StorableObjectFactory {
	/**
	 * @param transferable
	...
	protected StorableObject newInstance(final IDLEntity transferable) throws CreateObjectException {
		return new User((User_Transferable) transferable);
	}
	...
	protected Identifier getId(final IDLEntity transferable) {
		return new Identifier(((User_Transferable) transferable).header.id);
	}
	...
	protected IDLEntity[] allocateArrayOfTransferables(final int length) {
		return new User_Transferable[length];
	}
}
