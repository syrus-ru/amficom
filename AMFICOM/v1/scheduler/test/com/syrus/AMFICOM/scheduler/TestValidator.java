...
public final class TestValidator extends TestCase {
	...
	private static void getMeasurementSetups() throws ApplicationException {
		...
		final long longMsMeasurementDuration = LONG_MEASUREMENT_SETUP.calcTotalDuration();
		final long shortMsMeasurementDuration = SHORT_MEASUREMENT_SETUP.calcTotalDuration();
...
//				+ " sec");
	...
	public void testSelfIntersectionDueToChangeTemporalPattern() throws ApplicationException {
		...
		final Test test = createTestInstance(CREATOR_ID, 
			now, 
			end, 
	...
	public void testSingleTestsIntersection() throws ApplicationException {
		...
		final Test test;
		{
			...
			test = createTestInstance(CREATOR_ID, 
				start, 
				end, 
				temporalPattern != null ? temporalPattern.getId() : Identifier.VOID_IDENTIFIER, 
	...
	public void testSingleTestsIntersection2() throws ApplicationException {
		...
		final Test test;
		{
			final long measurementDuration = firstTestMeasurementSetup.calcTotalDuration();
		...
		// The second test's parameters:
		{
			...
			final Date start = new Date(now.getTime() - measurementDuration 
				+ firstTestMeasurementSetup.calcTotalDuration() / 2);
			final Date end = new Date(start.getTime() + measurementDuration);		
			final AbstractTemporalPattern temporalPattern = null;
	...
	public void testSingleTestsMovementIntersection() throws ApplicationException {
		...
		// The second test's parameters:
		{
			...
			final Test test2 = createTestInstance(CREATOR_ID, 
				start, 
				end, 
				temporalPattern != null ? temporalPattern.getId() : Identifier.VOID_IDENTIFIER, 
				TestTemporalType.TEST_TEMPORAL_TYPE_ONETIME, 
	...
	private static Test createTestInstance(Identifier creator_id,
			Date start,
			Date end,
			Identifier temporalPatternId,
			@SuppressWarnings("unused") TestTemporalType temporalType,
			Identifier analysisTypeId,
			Identifier groupId,
			MonitoredElement monitoredElement,
			String description,
			Set<Identifier> measurementSetupIds) throws ApplicationException {
		assert creator_id.equals(LoginManager.getUserId());
		return SchedulerModel.createTestInstance(
				description,
				groupId,
	...
	public void testSingleTestSelfIntersectionChangeMeasurementSetup() throws ApplicationException {
		...
		final long period = 5L * 60L * 1000L;
		assertTrue(measurementSetup2.calcTotalDuration() > period);
