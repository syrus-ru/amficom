...
public final class ElementsTreeFrame extends JInternalFrame implements PropertyChangeListener {
	...
	public MeasurementType getMeasurementType_orNull() {
		final JTree tree = this.treePanel.getTree();
		final TreePath treePath = tree.getSelectionPath();
		if (treePath != null) {
			for (int i = 0; i < treePath.getPathCount(); i++) {
		...
		this.schedulerModel.setBreakData();
		AbstractMainFrame.showErrorMessage(I18N.getString("Scheduler.Error.HaveNotChoosenMeasurementElement"));		
		return null; //MeasurementType.UNKNOWN;
	}
...
//		Log.debugMessage("monitoredElement " + (monitoredElement != null ? monitoredElement.getId() : null), Level.FINEST);
	...
	public void propertyChange(final PropertyChangeEvent evt) {
		...
		if (propertyName.equals(SchedulerModel.COMMAND_SET_MEASUREMENT_TYPE_OR_NULL)) {
		...
		} else if (propertyName.equals(SchedulerModel.COMMAND_GET_MEASUREMENT_TYPE_OR_NULL)) {
			final MeasurementType measurementType = getMeasurementType_orNull();
			if (measurementType != null) { // FIXME: maybe if(true) ?
				this.dispatcher.firePropertyChange(new PropertyChangeEvent(this,
						SchedulerModel.COMMAND_SET_MEASUREMENT_TYPE_OR_NULL,
						null,
						measurementType));
			}
	...
	public void init() {
				...
				public void selectedItems(final Collection<Item> items) {
					...
					for (final Item item : items) {
						...
						if (object instanceof Identifier) {
							...
							ElementsTreeFrame.this.schedulerModel.setSelectedMeasurementType_orNull((MeasurementType) object);
						}
					}
		...
		this.dispatcher.addPropertyChangeListener(SchedulerModel.COMMAND_GET_MEASUREMENT_TYPE_OR_NULL, this);
		this.dispatcher.addPropertyChangeListener(SchedulerModel.COMMAND_GET_MONITORED_ELEMENT, this);
		this.dispatcher.addPropertyChangeListener(SchedulerModel.COMMAND_SET_MEASUREMENT_TYPE_OR_NULL, this);
