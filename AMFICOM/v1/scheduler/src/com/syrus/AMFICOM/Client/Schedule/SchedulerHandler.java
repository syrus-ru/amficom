/*-
* $Id: SchedulerHandler.java,v 1.1.4.1 2006/04/11 07:42:55 saa Exp $
*
...
import com.syrus.AMFICOM.Client.Schedule.UI.MeasurementParametersPanel;
...
/**
 * @version $Revision: 1.1.4.1 $, $Date: 2006/04/11 07:42:55 $
 * @author $Author: saa $
...
public class SchedulerHandler extends AbstractExtensionHandler<SchedulerExtensions> {
	...
	private Map<String, MeasurementParametersPanel> parametersTestPanels;
	
	private final static String PARAMETERS_HANDLER_NAME = ParametersHandler.type.getName().getLocalPart();

	public SchedulerHandler() {
		this.parametersTestPanels = new HashMap<String, MeasurementParametersPanel>();
	}
	...
	public MeasurementParametersPanel getParametersTestPanel(final String id) {
		final MeasurementParametersPanel parametersTestPanel = this.parametersTestPanels.get(id);
		...
		for (final SchedulerResource resource : schedulerResourceArray) {
			if (resource.getId().equals(PARAMETERS_HANDLER_NAME)) {
				...
				if (id2.equals(id)) {						
					MeasurementParametersPanel testPanel = (MeasurementParametersPanel) 
