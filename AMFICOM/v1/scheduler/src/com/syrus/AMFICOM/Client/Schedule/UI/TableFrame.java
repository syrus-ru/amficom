				...
				public void mousePressed(MouseEvent evt) {
					...
					if (SwingUtilities.isRightMouseButton(evt)) {
						...
						for (final int index : rowIndices) {
							...
							final TestStatus status = testView.getTest().getStatus();
							if ((!testView.isTestNewer() || 
									status != TestStatus.TEST_STATUS_NEW) &&
								status != TestStatus.TEST_STATUS_PROCESSING &&
								status != TestStatus.TEST_STATUS_SCHEDULED &&
								status != TestStatus.TEST_STATUS_STOPPED) {
								return;
							}
						}
						...
						for (int i = 0; i < rowIndices.length; i++) {
							...
							final TestStatus status = testView.getTest().getStatus();
							if (!testView.isTestNewer() || status != TestStatus.TEST_STATUS_NEW) {
