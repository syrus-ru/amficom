/*
 * $Id: MCMConnectionManager.java,v 1.1 2005/03/31 16:04:13 arseniy Exp $
 * 
...
/**
 * @version $Revision: 1.1 $, $Date: 2005/03/31 16:04:13 $
...
public class MCMConnectionManager extends Thread {
	...
	public MCM getVerifiedMCMReference(Identifier mcmId) throws CommunicationException {
		if (!this.mcmIds.contains(mcmId)) {
			this.retrieveMCMIds();
			if (!this.mcmIds.contains(mcmId))
				throw new CommunicationException("MCM '" + mcmId + "' not registered for server '" + this.serverId + "'");
		}
