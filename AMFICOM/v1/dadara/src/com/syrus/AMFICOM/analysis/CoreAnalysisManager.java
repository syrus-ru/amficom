...
//				tpa.deltaX);
	 ...
	 *   строит пороги по исходной р/г (PFTrace) и (для страховки) ее а.к.
	 *   (хотя, в принципе, а.к. исх. р/г должна бы совпадать с а.к. эталона,
	 *   это все равно делается "на всякий случай", который иногда случается)
	...
	throws IncompatibleTracesException
	{
		...
		// расширяем по а.к. даже если исходная р/г одна - на случай
		// несоответствия эталонной а.к. и исходной. (страховочный вариант)
		//if (trColl.size() > 1)
		{
		...
		if (trColl.size() == 1) {
			...
			// XXX: use yTrace, not yCorr to workaround bug 326,
			// otherwise thresholds at DZ front can be too narrow if generated by one trace
			mtm.updateThreshToContain(av.av.yTrace, av.av.yTrace, MTM_DY_MARGIN,
