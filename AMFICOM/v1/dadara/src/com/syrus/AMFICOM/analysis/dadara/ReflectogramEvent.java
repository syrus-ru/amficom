...
public class ReflectogramEvent
{
	...
	{
		// не копируем ни mf, ни порогов, ни left-right связей
	...
	public ReflectogramEvent copy()
	{
		...
		ret.mf = mf.copy();
		// неизвестно, как быть с left-right связями
		return ret;
	}

	/**
	 * Создает копию массива событий, и устанавливает в копии естественные
	 * связи left-right, необходимые при использовании leftLink.
	 * Пороги новых событий указывают на пороги старых событий.
	 * @param re входной массив событий
	 * @return копия входного массива.
	 */
	...
	public static ReflectogramEvent[] fromByteArray(byte[] bar)
	{
		try
		{
			...
			// устанавливаем left-right связи
			createLeftRightLinks(ret);
			return ret;
		}
		catch (IOException e)
	...
	/**
	 * Проверяем, что модельная функция данного события
	 * лежит в пределах наших порогов,
	 * находим первую точку выхода за пороги.
	 * Начало и конец событий должны совпадать.
	 * @param that пробная модельная функция
	 * @param softAlarms указывает, что надо сравнивать с soft-порогами
	 *  (иначе сравнивается с hard-порогами)
	...
	/**
	 * Создаем событие с модельной функцией, получаемой
	 * из this линейной фитировкой по данному массиву.
	 * @param y рефлектограмма, по которой делается лин. фит.
