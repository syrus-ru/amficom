...
implements ReliabilityModelTraceAndEvents, DataStreamable {
	...
	private DetailedEvent makeDetailedEvent(int i) {
		...
		// Вычисление альтернативно вычисленного значения
		// если слева лин. событие хотя бы из 5 точек,
		// то альтернативным y0 будет экстраполированное значение по точке
		// непосредственно слева перед началом события;
		// иначе альтернативное y0 совпадает с y0

		// FIXME: плохая обусловленность на пороге срабатывания этого условия
		// Если слева - не лин. участок, то, наверное, надо брать min(y0, getY(begin-1)), т.к. резкого подъема перед отражением не бывает
		// @todo перед решением этих проблем посмотреть, как оно влияет на качество ЛТ в ЦБ
