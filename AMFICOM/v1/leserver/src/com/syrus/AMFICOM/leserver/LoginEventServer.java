/*
 * $Id: LoginEventServer.java,v 1.1 2005/04/28 10:33:36 arseniy Exp $
 * 
...
/**
 * @version $Revision: 1.1 $, $Date: 2005/04/28 10:33:36 $
 * @author $Author: arseniy $
 * @module loginserver_v1
 */
public final class LoginEventServer extends SleepButWorkThread {
	...
	public static final String PROCESS_CODENAME = "LEServer";
...
//			user = new User(serverProcess.getUserId());
		...
		/*	Activate servant*/
		try {
			SessionEnvironment.getLEServerServantManager().getCORBAServer().activateServant(new LoginServerImplementation(), codename);
		}
		catch (CommunicationException ce) {
			Log.errorException(ce);
			System.exit(1);
	...
	protected void shutdown() {
		...
		try {
			SessionEnvironment.getLEServerServantManager().getCORBAServer().deactivateServant(codename);
		}
		catch (CommunicationException ce) {
			Log.errorException(ce);
		}
