...
public class EmptyDataSource implements DataSourceInterface 
{
	...
	public EmptyDataSource(SessionInterface si)
	{
		this.si = si;
	}
	...
	public void LoadUserDescriptors()
	{
		...
		domain.created = 0;
		domain.created_by = "sys";
		domain.modified = 0;
		domain.modified_by = "sys";
		domain.domain_id = "";

		Pool.put("domain", domain.getId(), domain);
//		Pool.putName("domain", domain.getId(), domain.getName());

		user = new User();

		user.id = "sys";
		user.login = "sys";
		user.logged = System.currentTimeMillis();
		user.object_id = "sys";
		user.sessions = 2;
		user.last_login = 0;
		user.type = "operator";
...
//		Pool.putName("user", user.getId(), user.getName());
	...
	public void LoadTestTypeDescriptors()
	{
	}
