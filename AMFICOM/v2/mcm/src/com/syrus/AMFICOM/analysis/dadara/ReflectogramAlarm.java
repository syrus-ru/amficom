...
public class ReflectogramAlarm {
	...
	public int eventType = 0; // 0 -linear; 3 -weld; 4 -connector.
  ...
  public ReflectogramAlarm(int alarmPointCoord, int level, int alarmEndPointCoord)	{
	...
	public ReflectogramAlarm(byte[] bar) {
		...
		try {
			...
			Log.errorMessage("Exception while converting byte array to AlarmParams");
	...
	public byte[] getByteArray() {
		...
		try {
			...
			Log.errorMessage("Exception while getting byte array from alarm");
	...
	public static ReflectogramAlarm[] fromByteArray(byte[] bar) {
		...
		try {
			...
			Log.errorMessage("Exception while converting byte array to array of alarms");
			Log.errorException(ioe);
			return null;
		}
