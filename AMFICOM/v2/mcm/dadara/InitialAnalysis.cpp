...
void InitialAnalysis::setEventsTble()
{
	...
	for(i=0; i<data_length; i++) eventsTable[i] = 0;
    ...
    for(i=0; i<lastNonZeroPoint && i<data_length-10; i++)
     {
       x1 = i;
       j=i+1;
	   type = 0;
...
void InitialAnalysis::excludeNonRecognizedEvents()
{
	...
	if(na->readWeightsAndShiftsFromSingleFile(fileName))
	{
		for(int i=3; i<numberOfFoundEvents*3-3; i+=3)
		{
			...
			{
				setNNWindow(eventsTable[i+1], eventsTable[i+2], window);
	...
	{   
		printf("ERROR: Weights are not set.\n");
	}
...
void InitialAnalysis::setCorrectEndingSplash()
{
	...
	for(i=numberOfFoundEvents-1; i>=0; i--)
	{
		...
		{
			ep[i]->endConnector = ep[i]->endLinear = ep[i]->endWeld = ep[i]->beginConnector+eventSize*3;
...
void InitialAnalysis::setNNWindow(int from, int to, double *window)
{
	...
	double dIndex = (double)(myTo - myFrom)/66.;
	
	double counter = myFrom;


	for(int i=0; i<66; i++)
	{
		window[i] = data[(int)(counter+0.5)] - meanAttenuation*(counter-myFrom)/eventSize;
		counter += dIndex;

	}

	shiftEventToCentre(window, 66);
	return;
}
