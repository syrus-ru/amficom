 ...
 * @module util
 * @deprecated This is an old implementation of class {@link com.syrus.util.LRUMap}.
 */
@Deprecated
public class ArrayLRUMap<K, V> implements Serializable, Iterable<V> {
	...
	public ArrayLRUMap(final int capacity) {
		if (capacity >= 0) {
	...
	public String toString() {
		...
		for (int i = 0; i < this.entityCount; i++) {
			...
			if (key == this) {
				stringBuffer.append("This ArrayLRUMap");
			} else {
	...
	private abstract class AbstractIterator {
		 ...
		 */
		int expectedModCount = ArrayLRUMap.this.modCount;

		public final boolean hasNext() {
			return this.cursor != ArrayLRUMap.this.entityCount;
		}
		...
		public final void remove() {
			...
			try {
				ArrayLRUMap.this.remove(ArrayLRUMap.this.array[this.lastRet].getKey());
				if (this.lastRet < this.cursor) {
					this.cursor--;
				}
				this.lastRet = -1;
				this.expectedModCount = ArrayLRUMap.this.modCount;
		...
		final void checkForComodification() {
			if (ArrayLRUMap.this.modCount != this.expectedModCount) {
				throw new ConcurrentModificationException();
			}
		}
	}
	...
	private class Itr extends AbstractIterator implements Iterator<V> {
		public V next() {
			this.checkForComodification();
			try {
				final IEntry<K, V> nextEntry = ArrayLRUMap.this.array[this.cursor];
