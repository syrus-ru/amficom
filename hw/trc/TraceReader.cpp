...
int fill_bellcore_structure(const char* compdata, const unsigned short compdata_size, BellcoreStructure* bs) {
	...
	bs->genParams->FID = " ";
	bs->genParams->NW = nttrace_data->_00014;
	bs->genParams->CMT = "QuestFiber";

	bs->addField(BellcoreStructure::SUPPARAMS);
	bs->supParams->MFID = "Nettest Starprobe";
	i = strlen(nttrace_data->_00084);
	strncpy((char*)bs->supParams->OMID, nttrace_data->_00084, i + 1);
	...
	bs->fxdParams->DS[0] = (int)(nttrace_data->_40072 * nttrace_data->_00012 / 3. * 100);
	bs->fxdParams->NPPW = new int[1];	//!
	bs->fxdParams->NPPW[0] = nttrace_data->_40066;
	bs->fxdParams->GI = nttrace_data->_00012 * 10;
...
//	double km_per_data_point = (nttrace_data->_40072 * dbl_10050458)/(nttrace_data->_00012 / dbl_100504F0);
	...
	bs->dataPts->TSF = 1;
	bs->dataPts->TNDP = nttrace_data->_40066;
	bs->dataPts->TPS = new int[1];					//!
	bs->dataPts->SF = new short[1];					//!
	bs->dataPts->TPS[0] = nttrace_data->_40066;
	bs->dataPts->SF[0] = 1000;
	bs->dataPts->DSF = new unsigned short*[1];			//!
...
//---------
	...
	bs->map->MRN = 100;
	bs->map->NB = 6;
	bs->map->B_id = new const char*[6];		//!
	bs->map->B_rev = new unsigned short[6];	//!
	bs->map->B_size = new int[6];		//!
	bs->map->B_id[0] = BellcoreStructure::MAP_STR;
	bs->map->B_id[1] = BellcoreStructure::GENPARAMS_STR;
	bs->map->B_id[2] = BellcoreStructure::SUPPARAMS_STR;
	bs->map->B_id[3] = BellcoreStructure::FXDPARAMS_STR;
	bs->map->B_id[4] = BellcoreStructure::DATAPOINTS_STR;
	bs->map->B_id[5] = BellcoreStructure::CKSUM_STR;
	for (i = 0; i < 6; i++)
		bs->map->B_rev[i] = 100;
	bs->map->B_size[1] = bs->genParams->get_size();
	bs->map->B_size[2] = bs->supParams->get_size();
	bs->map->B_size[3] = bs->fxdParams->get_size();
	bs->map->B_size[4] = bs->dataPts->get_size();
	bs->map->B_size[5] = bs->cksum->get_size();
	bs->map->MBS = bs->map->get_size();
